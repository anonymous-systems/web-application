 ...
 * collections are small, so loading them whole costs less than resolving links
 * one at a time.
 */
export const loadTerms = async (): Promise<Map<string, TaxonomyTermRef>> => {
  const snapshots = await Promise.all(
    COLLECTIONS.map((path) => db().collection(path).get())
  )
